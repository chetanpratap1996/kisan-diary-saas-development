...
export async function localUpdateUser(userId: string, patch: Partial<Pick<LocalUser, "name" | "state" | "district" | "village" | "pincode" | "pmKisanId" | "language" | "phone" | "username" | "passwordHash" | "isAdmin">>) {
  const store = await readStore();
...
export async function localCreateFarm(input: { userId: string; name: string; sizeAcre: number; location: string; landType?: string; irrigationSource?: string; soilType?: string }) {
  const store = await readStore();
  const now = new Date().toISOString();
...
export async function localUpdateFarm(
  id: string,
  userId: string,
  patch: Partial<Pick<LocalFarm, "name" | "sizeAcre" | "location" | "landType" | "irrigationSource" | "soilType">>
) {
  const store = await readStore();
